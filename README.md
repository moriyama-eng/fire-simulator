...
- 共通の乱数シード値とシミュレーション回数を全シナリオに適用できます。
- 英語モードでは通貨単位が自動変換され（100円 = $1 固定レート）、入力フィールドの横に「K」「M」などの単位ラベルが表示されます。
- 現金バッファまたはガードレールがOFFのシナリオでは、関連するパラメータ行は自動的に非表示になります。
- テーブル表示は最初のシナリオの設定に基づくため、全シナリオで現金バッファ/ガードレール設定を統一することを推奨します。
...
- **ユニットテスト**（`tests/unit/` — 10ファイル）: 純粋関数・コアロジックの正しさを検証
- **インテグレーションテスト**（`tests/integration/` — 4ファイル）: DOM操作・UI状態遷移を検証
